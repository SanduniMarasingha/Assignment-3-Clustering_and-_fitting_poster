...
for ncluster in range(2, 10):
    ...
    kmeans.fit(df_forest_co2_2017[['Forest Area','CO2 Emmision']]) # fit done on x,y pairs
    labels = kmeans.labels_
    # extract the estimated cluster centres
    cen = kmeans.cluster_centers_
    # calculate the silhoutte score
    print(ncluster, skmet.silhouette_score(df_forest_co2_2017[['Forest Area','CO2 Emmision']], labels))
...
df_forest_co2_2017_numeric = df_forest_co2_2017.select_dtypes(include=[np.number])
...
df_forest_co2_2017_norm = pd.DataFrame(df_forest_co2_2017_norm, columns=df_forest_co2_2017_numeric.columns)

# Add the non-numeric columns back to the DataFrame
df_forest_co2_2017_norm = pd.concat([df_forest_co2_2017[df_forest_co2_2017.columns.difference(df_forest_co2_2017_numeric.columns)], df_forest_co2_2017_norm], axis=1)
...
df_forest_Sri = df_forest_Sri.rename(columns={'index':'Year','Sri Lanka':'Forest Area'})
...
popt, pcorr = opt.curve_fit(exp_growth, df_forest_Sri["Year"],df_forest_Sri["Forest Area"])

print("Fit parameter", popt)

df_forest_Sri["pop_exp"] = exp_growth(df_forest_Sri["Year"], *popt)
plt.figure()
plt.plot(df_forest_Sri["Year"], df_forest_Sri["Forest Area"], label="data", color = '#458B00')
plt.plot(df_forest_Sri["Year"], df_forest_Sri["pop_exp"], label="fit", color ='#DC143C')
...
# Fit the logistic model to the data
p0 = [max(df_forest_Sri["Forest Area"]), 1, np.median(df_forest_Sri["Forest Area"])]
params, covar = optimize.curve_fit(logistic, df_forest_Sri["Forest Area"], df_forest_Sri["pop_logistics"], p0)
...
clustered_countries = df_forest_co2_2017.groupby('cluster').apply(lambda x: x.sample(n=1))
...
cluster0_countries = df_forest_co2_2017[df_forest_co2_2017['cluster'] == 0].sample(n=3)
cluster1_countries = df_forest_co2_2017[df_forest_co2_2017['cluster'] == 1].sample(n=3)
